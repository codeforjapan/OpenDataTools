...
describe('utils/multibyte', () => {
  test('全角英数字から半角英数字', () => {
    const test1 = new Moji('１０９０１２３').convert('alphanumeric', 'ZtoH');
    expect(test1).toEqual('1090123');
    const test2 = new Moji('ＡＺmo204４3').convert('alphanumeric', 'ZtoH');
    expect(test2).toEqual('AZmo20443');
  });
  test('半角英数字から全角英数字', () => {
    const test1 = new Moji('1090123').convert('alphanumeric', 'HtoZ');
    expect(test1).toEqual('１０９０１２３');
    const test2 = new Moji('Azmo20443').convert('alphanumeric', 'HtoZ');
    expect(test2).toEqual('Ａｚｍｏ２０４４３');
  });
  test('半角カナから全角カナ', () => {
    const test1 = new Moji('｢ﾓﾄｸﾛｽﾊﾞｲｸ｣').convert('kana', 'HtoZ');
    expect(test1).toEqual('「モトクロスバイク」');
    const test2 = new Moji('ﾃﾞｨｽﾞﾆ-ランﾄﾞ').convert('kana', 'HtoZ');
    expect(test2).toEqual('ディズニーランド');
  });
  test('全角カナから半角カナ', () => {
    const test1 = new Moji('「モトクロスバイク」').convert('kana', 'ZtoH');
    expect(test1).toEqual('｢ﾓﾄｸﾛｽﾊﾞｲｸ｣');
    const test2 = new Moji('ディズニーﾗﾝド').convert('kana', 'ZtoH');
