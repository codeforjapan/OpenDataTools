class ItemValidatorTel {
  validateDataType(data: any) {
    ...
    if (
      ...
      !matchesDomesticPhone &&
      !matchesInternationalPhone
    ) {
      throw new Error('電話番号は国内電話の場合は市外局番から始まり、国際電話の場合は「+国番号」から始まる必要があります。セパレータには半角ハイフン「-」を使用します。');
    }

    if (
      matchesNumber && (matchesNumber.length < 10 || matchesNumber.length > 15)
    ) {
